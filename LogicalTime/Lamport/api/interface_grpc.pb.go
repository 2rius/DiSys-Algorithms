...
func (c *chatClient) Connect(ctx context.Context, opts ...grpc.CallOption) (Chat_ConnectClient, error) {
	stream, err := c.cc.NewStream(ctx, &Chat_ServiceDesc.Streams[0], "/Lamport.Chat/Connect", opts...)
	if err != nil {
		return nil, err
	}
...
type Chat_ConnectClient interface {
	Send(*MsgClient) error
	Recv() (*MsgServer, error)
	grpc.ClientStream
}
...
func (x *chatConnectClient) Send(m *MsgClient) error {
	return x.ClientStream.SendMsg(m)
}

func (x *chatConnectClient) Recv() (*MsgServer, error) {
	m := new(MsgServer)
...
type Chat_ConnectServer interface {
	Send(*MsgServer) error
	Recv() (*MsgClient, error)
	grpc.ServerStream
}
...
func (x *chatConnectServer) Send(m *MsgServer) error {
	return x.ServerStream.SendMsg(m)
}

func (x *chatConnectServer) Recv() (*MsgClient, error) {
	m := new(MsgClient)
